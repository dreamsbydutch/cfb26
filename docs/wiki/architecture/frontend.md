...
| `/admin/roster` | `src/routes/admin.roster.tsx` | No-index single-owner movement desk with a mobile task dropdown and collapsible in-memory access-key control. Wider screens retain the task rail beside the complete edit/add/remove workflows.                                               | `rosters.list`, `players.getProfile`, `teamData.listPrograms`, and three `rosterAdmin` mutations. |
