...
The Games list uses one responsive expandable row on all screen sizes. Upcoming broadcast windows lead, started games follow, and past results appear last with the newest first. Final scores require confirmed completion and both scores; elapsed kickoff alone never implies a final or live score. A browser timer refreshes grouping every 30 seconds. Kickoffs over eight hours old without completion are labeled Result pending. Power values, venue, interest scores, and upcoming projections sit inside keyboard-operable details; team names wrap instead of truncating.
