...
| --- | ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| `/` | `src/routes/index.tsx` | Client-rendered responsive personnel explorer with offense, defense, and special-teams depth tabs whose position rooms show starters, rotation, prospects, and walk-ons together, plus loading, error, empty, and detail UI. | `rosters.list`, `players.getProfile`, `seasonalStats.listBySeason`. |
...
- The depth chart uses offense, defense, and special-teams tabs. Each tab keeps its full position rooms in one view, and each room separates starters, rotation, prospects/transfers, and walk-ons into consistent columns. The first recorded players fill a base 11-personnel offense, base defense, and K/P/LS unit. The checked-in [2015–2025 position-workload report](../reference/snap-count-position-workload-report.md) sizes broad QB, HB, WR, OL, IDL, EDGE, LB, and DB rotations; K, P, and LS each include the next listed player in rotation. Remaining active players are split into prospects/transfers and walk-ons from the recruiting profile source. The UI explicitly avoids presenting these tiers as official starts or snap projections.
