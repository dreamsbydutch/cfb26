...
National season and week controls scroll with the page. All three ranking tables share the same dense headers, separators, short rows, and content-sized maximum width so values remain easy to trace back to a team on wide screens. Program replaces the generic record with compact 25-season national- and conference-title markers; each mark fades with age and disappears outside that window, and conference marks stack two high with a Big Ten-specific logo when the retained title identifies that conference. Narrow screens initially show rank, team, and the primary rating, then use horizontal scrolling for rating-specific evidence: Power units, Program honors and score allocations, or Résumé record quadrants and contributions.
