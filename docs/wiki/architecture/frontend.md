...
| --------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `/`             | `src/routes/index.tsx`        | Client-rendered mobile-first personnel explorer. Narrow screens use a persistent Roster/Games bottom navigation, a roster-view dropdown, compact expandable position/class groups, and bottom-sheet player profiles; wider screens retain tabs, multi-column layouts, and side-sheet details. | `rosters.list`, `players.getProfile`, `seasonalStats.listBySeason`.                               |
| `/games`        | `src/routes/games.tsx`        | Mobile-first national landscape dashboard. Narrow screens use dropdown controls, compact expandable game and ranking rows, and rating detail sheets while preserving the points-scale Power ranking and three game-order lenses.                                                              | `ratings.getWeeklyDashboard`, `ratings.getMatchup`.                                               |
| `/admin/roster` | `src/routes/admin.roster.tsx` | No-index single-owner movement desk with a mobile task dropdown and collapsible in-memory access-key control. Its in-season editor is limited to jersey number, position, position order, and depth section; wider screens retain the task rail.                                              | `rosters.list`, `players.getProfile`, `teamData.listPrograms`, and three `rosterAdmin` mutations. |
...
- Recruit and draft classes render as compact season disclosures. Opening one shows its table and positional context. Recruit tables include player, recruit-time position, stars, Composite rating, 247 rating, overall ranks, and acquisition type. A two-option control ranks every visible class by either rating source; missing ratings sort last and remain explicitly unrated. Narrow screens prioritize rank, player, stars, and the selected rating, while tablet widths restore the secondary columns. Wide screens pair each recruit table with a recruit-time positional breakdown rather than stretching the table across the full content width.
...
- Player profiles derive career totals and peak seasons from the same seasonal records, then show every Michigan season in a compact production table. The shared modal renders as a bottom sheet on narrow screens and a side sheet on wider screens, traps keyboard focus, supports Escape/backdrop dismissal, restores prior focus, locks background scrolling, and respects the mobile safe area. It keeps player facts to position, height, weight, high school, and combined hometown/state; eligibility-window and admin-override fields remain background data. Recruiting details emphasize the original class and star level, followed by aligned Composite and 247 rating, overall-rank, position-rank, and state-rank rows. Stars use the Composite overall/rating bands when present, fall back to the 247 rating band, and otherwise display as unrated.
...
The admin route also opts out of SSR. It reads the same bounded active roster plus the bounded canonical-program list used by transfer school selectors. Narrow screens use a native task dropdown and collapsible access-key control; wider screens restore the task rail and persistent key panel. The searchable in-season editor exposes only jersey number, current position, one-based position order, and an optional Starter/Rotation/Depth/Prospect/Walk-on section override. A position change automatically appends the current season to public position history; eligibility and injury data remain unchanged. Add dynamically asks for the fields relevant to a recruit, transfer, or walk-on and keeps optional recruiting ratings collapsed. Remove requires an explicit confirmation and explains that history remains intact. Loading, empty, mutation-success/error, and route-error states are handled in the feature.

The shared key is intentionally not stored in local/session storage or placed in the URL; a refresh clears it. All three typed mutations receive it and verify it server-side before reading data. This is a single-owner control documented in [ADR 0005](../decisions/0005-single-owner-roster-admin-key.md), not a user login. The checked-in narrowed update call and add/remove calls require the matching Convex functions to be pushed before those tasks work against a hosted deployment.

The landscape route keeps `ratings.getWeeklyDashboard` reactive to season/week. There is one predictive ordering: CFB26 Power Rank. Narrow screens replace view and game-order button groups with native dropdowns, render games and rankings as compact rows, expand game context inline, and open a focus-trapped rating detail sheet on team selection; the full evidence table remains available on wide screens. Power, offense, defense, special teams, team-specific home field, games played, prior influence, sources, and limited-sample status remain available, while Week 7 Résumé appears only as supporting evidence. Game ordering stays client-side over the bounded response and offers three independent lenses: matchup quality, national playoff importance, and Michigan importance. Optional CFBD television outlets appear inside each game disclosure. The matchup tab enables a second `ratings.getMatchup` read only when two distinct modeled teams are selected; venue is part of that query key. Matchup unit edges, rating profiles, and stored series history use compact disclosures. Ranking and projection methodology lives in [Proprietary team ratings and game importance](../reference/landscape-ranking.md).
...
The explorer uses Michigan blue (`#00274C`) and maize (`#FFCB05`) with restrained pale tints, compact tables and rows, sans-serif headings, and progressive disclosure. The public roster and games routes share a fixed, safe-area-aware two-tab bottom navigation on narrow screens. Narrow screens otherwise prefer native dropdowns, expandable summaries, bottom sheets, and touch targets of roughly 44 pixels; wider screens progressively restore button groups, tables, rails, and side sheets. Blue anchors the header, headings, and active controls; maize marks emphasis, selection, and focus. Backgrounds remain flat and gradients are not used. Sections and dividers take priority over cards.
