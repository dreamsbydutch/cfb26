...
| `/games`        | `src/routes/games.tsx`        | Responsive national landscape dashboard. Users select season/week, browse weekly importance, compare points-scale Power and Week 7 Résumé rankings, or build venue-aware head-to-head comparisons with unit ratings and series history.                                 | `ratings.getWeeklyDashboard`, `ratings.getMatchup`.                                               |
...
The landscape route keeps `ratings.getWeeklyDashboard` reactive to season/week. Game sorting by national or Michigan importance and Power/Résumé sorting stay client-side over its bounded response. Power remains the default; Résumé controls and values remain hidden before Week 7. The table shows both ranks, their difference, points-scale offense/defense, limited-sample status, edition vintage, and disagreement reasons. The matchup tab enables a second `ratings.getMatchup` read only when two distinct modeled teams are selected; venue is part of that query key. Ranking and projection methodology lives in [Proprietary team ratings and game importance](../reference/landscape-ranking.md).
