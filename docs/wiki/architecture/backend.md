...
`convex/schema.ts` declares 44 tables and 106 indexes. The principal ownership seams are:
...
The shared [live scoreboard](../reference/live-scoring.md) stores compact observations on college games and reserves request budgets in `scoreboardSyncState`. Its five-minute internal cron is gated by deployment enablement, game windows, account allowance, and backoff; it does not run the full daily ingestion pipeline.
