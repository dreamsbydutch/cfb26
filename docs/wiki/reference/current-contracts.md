...
| `/games`, `/national/*`            | Games, continuous complete-field Power, Résumé, deterministic Playoff, Teams/program profiles, Simulator, Blind Ballot, and Methodology. Mobile game cards prioritize the matchup, selected importance, kickoff, venue, and television outlet, with projections in expandable detail; desktop retains the analytical schedule table. Schedule ranks come from the selected edition. `/national/games` redirects to `/games`.               |
