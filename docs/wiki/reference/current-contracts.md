...
- Output: the resolved season/week, selected immutable edition metadata, up to 200 published points-scale Power/Résumé rows, and up to 200 FBS games decorated with Power ranks, national importance, Michigan importance, and a Michigan relationship label. Résumé values remain hidden before Week 7. Reads fall back to the legacy composite and then Elo only when no new edition exists.
