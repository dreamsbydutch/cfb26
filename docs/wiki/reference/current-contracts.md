...
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /`             | Renders the mobile-first Michigan personnel explorer with six primary views and a shared mobile bottom navigation for switching between Roster and Games. Narrow screens use a view dropdown, expandable position/recruit/draft groups, and bottom-sheet player profiles; wider screens restore tab navigation, multi-column layouts, and side-sheet details. Depth rooms retain continuous tables grouped into starters, rotation, depth, first/second-season prospects, and walk-ons. Green, yellow, and red row backgrounds communicate eligibility bands; jersey numbers lead each row and injuries use one, two, or three inline red medical crosses. Recruit tables can rank players by Composite or 247 rating and keep unrated players last. | `rosters.list`, `players.getProfile`, `seasonalStats.listBySeason`.                                                                             |
| `GET /games`        | Renders the mobile-first national landscape dashboard with season/week controls, one evidence-rich points-scale Power Rank, game ordering by matchup quality, playoff importance, or Michigan importance, optional TV outlets, a matchup lab, and the shared mobile Roster/Games bottom navigation. Narrow screens use native control dropdowns and compact game/ranking rows. Games and matchup factors expand inline; selecting a ranking opens a focus-trapped detail sheet with unit ratings, sample context, evidence sources, and Week 7 Résumé support. Wide screens retain the evidence table.                                                                                                                                               | `ratings.getWeeklyDashboard`, `ratings.getMatchup`.                                                                                             |
| `GET /admin/roster` | Renders the no-index single-owner roster movement desk. Narrow screens use a native task dropdown and collapsible in-memory access-key control; wider screens retain the task rail and key panel. Edit limits searchable active-player maintenance to jersey number, position, position order, and depth section. Add records a recruit, transfer, or walk-on with identity, origin, roster/recruiting measurements, separate arrival/eligibility clocks, and optional ratings; Remove records a confirmed transfer, graduation, retirement, or dismissal without deleting history. Every write is verified by Convex.                                                                                                                               | `rosters.list`, `players.getProfile`, `teamData.listPrograms`, `rosterAdmin.updatePlayer`, `rosterAdmin.addPlayer`, `rosterAdmin.removePlayer`. |
...
- Output: roster stints joined to canonical player documents. Returned stints use a five-season eligibility baseline plus medical and admin-granted extra seasons, omit the legacy redshirt field, and may include a one-based position order, depth-tier override, current injury, and bounded position-change history.
...
- Kind: mutation.
- Input: the player/program identity, nullable jersey number, current position, nullable one-based position order, nullable depth-tier override, effective season, and `adminKey`.
- Output: the normalized updated roster stint.
- Validation: jersey numbers are whole numbers from 0–99, position order is a whole number from 1–99, position labels and seasons are bounded, and position changes append to a maximum 20-entry history. Eligibility and injury fields are not accepted or changed. The update is atomic.
...
- Environment: the earlier development function is deployed, but this narrowed contract still needs an authorized development push. Writes then remain disabled until that deployment key is configured. Production promotion and a separate production key are pending.
