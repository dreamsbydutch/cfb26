...
- Ranking tables share compact schedule-style rows and a content-sized maximum width. They lead with rank, team, and the named rating. Power exposes its record plus offense, defense, and available special-teams components. Program omits record, shows cutoff-safe national- and conference-title marks from the latest 15 seasons with age-based fading, and exposes its 50/15/5/30 results, acquisition, development, and honors allocations. Résumé alone exposes Q1–Q4 records alongside its weighted results, performance, history, and season-strength evidence. Model diagnostics and edition basis remain on Methodology rather than in every row.
