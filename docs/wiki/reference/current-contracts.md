...
| `/games`, `/national/*`            | Games, continuous complete-field Power, Résumé, deterministic Playoff, Teams/program profiles, Simulator, Blind Ballot, and Methodology. Games use fuzzy Eastern broadcast windows while retaining exact local kickoff times. The schedule has two game ratings: Landscape for nationally important, strong, close matchups, and Michigan for Wolverines schedule, rivalry, rank-band, conference, and shared-opponent relevance across the season. Live and started games lead, followed by upcoming games and past results. Live rows show scores, period/clock, and freshness from the shared five-minute scoreboard refresh. Completed games show final scores and winners; missing results remain explicitly pending. One responsive expandable row shows teams and kickoff or final score, with upcoming TV outlets visible. A muted negative projected spread appears beside the favored team for upcoming games. Power, interest scores, venue, and the full home-margin projection are in details. `/national/games` redirects to `/games`. |
| `/admin/roster`, `/admin/roster/*` | No-index desktop Owner Dashboard, inline-editable Roster/player editor, guided Player Stats entry, Data/import/identity, and isolated Operations routes. Saved game stats are preloaded for in-season updates; advanced lifecycle, annual fields, and bulk import remain secondary. A password creates a revocable 12-hour session; all access validates server-side, and narrow layouts expose status/sign-out without mutation controls.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

All routes include responsive navigation, semantic controls, visible focus treatment, and explicit loading/error/empty states. All three workspaces use white foundations. Michigan uses navy structure with maize highlights; National and Owner use graphite and grayscale accents, with Michigan colors reserved for Michigan programs and matchups inside national lists.
...
- batch-edit jersey, position, role, status, availability, games played, and starts before one validated transaction;
...
- Ranking tables share compact schedule-style rows and a content-sized maximum width. They lead with rank, team, and the named rating. Power exposes its record plus offense, defense, and available special-teams components. Program omits record, shows compact cutoff-safe national- and conference-title marks from the latest 25 seasons with age-based fading, stacks conference marks two high, and uses retained conference identity for league-specific art. Its four evidence columns expose raw 0–100 results, acquisition, development, and honors scores; the overall rating weights them 50/20/10/20. Résumé alone exposes Q1–Q4 records alongside its weighted results, performance, history, and season-strength evidence. Model diagnostics and edition basis remain on Methodology rather than in every row.
- Résumé appears from Week 7 and uses the approved 70% results plus 30% capped-performance split, followed by the bounded own-history bonus.
