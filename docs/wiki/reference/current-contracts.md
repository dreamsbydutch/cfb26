...
`VITE_CONVEX_URL` may override the browser deployment. When omitted, the app uses the public development URL `https://adjoining-opossum-710.convex.cloud`, which exposes the proprietary model and earlier update mutation. `CFB26_ADMIN_KEY` belongs only in each target Convex deployment and enables the roster mutations installed there. The checked-in arrival/departure mutations need an authorized development push; production still needs the checked-in 19-table contract, functions, snapshots, and a separately chosen admin key promoted before it exposes the same behavior. See [Configuration](configuration.md) and [Deployment](../guides/deployment.md).
