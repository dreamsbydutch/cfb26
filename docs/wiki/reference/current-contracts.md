...
| `/games`, `/national/*`            | Games, continuous complete-field Power, Résumé, deterministic Playoff, Teams/program profiles, Simulator, Blind Ballot, and Methodology. Games use fuzzy Eastern broadcast windows while retaining exact local kickoff times. The schedule has two game ratings: Landscape for nationally important, strong, close matchups, and Michigan for Wolverines schedule, rivalry, rank-band, conference, and shared-opponent relevance across the season. Venue and television remain visible; mobile projections expand on demand while desktop adds dedicated projection and location columns. `/national/games` redirects to `/games`. |
