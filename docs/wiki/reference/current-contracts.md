...
| `/games`, `/national/*`            | Games, continuous complete-field Power, Résumé, deterministic Playoff, Teams/program profiles, Simulator, Blind Ballot, and Methodology. Games use fuzzy Eastern broadcast windows while retaining exact local kickoff times. A continuous compact schedule keeps matchup, selected importance, venue, and television visible; mobile projections expand on demand while desktop adds dedicated projection and location columns. Schedule ranks come from the selected edition. `/national/games` redirects to `/games`. |
