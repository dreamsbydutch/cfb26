...
The active `cfb26-power-v3` uses opponent-adjusted points and five-season recursive history. Current-season games have equal calendar weight. Historical carryover now has four effective games of weight, down from eight in v2, and remains two when crossing from FCS to FBS. Thus the historical share of the prior-plus-games diagnostic is 80% after one game, 50% after four, and 25% after twelve, instead of 89%, 67%, and 40%. This diagnostic is not an exact decomposition of influence: ridge regularization, opponent fitting, and subdivision pooling also contribute.
...
`powerResearch.ts` retains archived offseason-transition and 60-/120-day recency challengers. Turnover evidence can reduce prior confidence when observed before the cutoff. Reconstructed weekly forecasts freeze before the week's first kickoff, with a conservative six-hour completion buffer for earlier games. `npm run ratings:evaluate -- <model-data.json> <new-report.json>` compares the published v3 policy against v2, refuses to overwrite reports, and applies the eight-season promotion gate. It never changes the deployed model automatically. Production and research share the carryover and margin-fit settings; regression tests reconstruct both paths identically. Research applies alternative carryover settings to subdivision-adjusted models and includes all policy settings in its historical-cache key.
