...
- Users can browse by original recruiting class, NFL entry class, exact Michigan position, 2015–2025 snap-count/PFF season, or the full searchable player index and open a detailed bottom/side sheet. Public routes use compact rows, progressive disclosure, and a persistent mobile Roster/Games switcher as the mobile-first interaction model.
...
- `/admin/roster` limits in-season player maintenance to jersey number, position, position order, and depth section, while keeping recruit/transfer/walk-on arrivals and confirmed departures as separate workflows. Additions create the normalized player lifecycle atomically; removals retain history. The checked-in mutation shapes still need a development push, and every write remains disabled until that deployment also has a Convex admin key.
