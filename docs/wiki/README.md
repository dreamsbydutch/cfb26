...
**Current:** `cfb26` is a three-route TanStack Start application. `/` is a responsive Michigan football personnel explorer; `/games` combines weekly importance, proprietary multi-perspective rankings, and custom matchup projections; `/admin/roster` is a no-index, deployment-key-gated movement desk for player maintenance, complete recruit/transfer/walk-on arrivals, and history-preserving departures. Development has the 19-table model with 2000–2026 composite snapshots and 2025–2026 advanced inputs, but the newly checked-in arrival/departure mutations still need an authorized development push. Production remains on the prior 17-table, 47,774-document foundation until an explicit promotion. The owner-confirmed Vercel project and production domain are recorded, but the Nitro-backed web deployment still needs a production smoke check. The app has no identity provider, automated tests, or CI workflow.
